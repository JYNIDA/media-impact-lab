...
def generate_digest(api_key, brief=""):
    ...
    system = (
        ...
        "각 항목 끝에 *발행일*을 `(YYYY-MM-DD)` 형식으로 반드시 표기해(검색 결과의 "
        "게재일 기준, 정확한 날짜를 모르면 `(날짜 미상)`). "
        "되도록 최신 기사를 우선하되, 윈도우 밖의 오래된 기사라도 다큐에 정말 탁월하게 "
        "유용한 '마스터피스'면 포함해 — 대신 발행일을 분명히 남겨서 오래된 글임을 알 수 있게 해."
    )
